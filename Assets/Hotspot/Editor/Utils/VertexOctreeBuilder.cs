...
namespace Hotspot.Editor
{
    ...
    public class VertexOctreeBuilder
    {
        ...
        public bool CreateOctree(bool usePlayModeCache = false)
        {
            ...
            var meshes = filters
                .Where(x => x.gameObject.TryGetComponent<Renderer>(out var renderer) &&
                            !LODRendererCache.IsLOD(renderer))
                .ToArray();
        ...
        public class OctreeNode
        {
            ...
            private void Split()
            {
                ...
                _children[0] =
                    new OctreeNode(
                        new Bounds(
                            new Vector3(_bounds.center.x - offset, _bounds.center.y - offset,
                                _bounds.center.z - offset), new Vector3(childSize, childSize, childSize)), _maxPoints,
                        _minSize, _vertsPerPixelThreshold);
                _children[1] =
                    new OctreeNode(
                        new Bounds(
                            new Vector3(_bounds.center.x + offset, _bounds.center.y - offset,
            ...
            public int GetVertsPerPixelHotSpots(in ICollection<Mesh> meshes)
            {
                ...
                else
                {
                    if (_vertices.Count > 0 && _originMeshes.ContainsAny(meshes))
                        return Convert.ToInt32(_vertices.Count / _bounds.GetScreenPixels(Camera.main) >
                                               _vertsPerPixelThreshold);
                }
